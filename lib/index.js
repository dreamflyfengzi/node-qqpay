...
class Payment {
  constructor({
    ...
    this.urls = sandbox ? {
      ...
      sendredpack: 'https://api.qpay.qq.com/cgi-bin/hongbao/qpay_hb_mch_send.cgi',
  ...
  static async sandbox(config, debug) {
    let {
      sandbox_signkey
    } = await Payment.init(config).getSignkey();
    return new Payment({
      ...config,
  ...
  async _parseJson(data, type) {
    let json = JSON.parse(data);
    if (json.retcode !== "0") throw new Error(json.retmsg || 'JSONDataError' + data);
    return json;
  }
  async _parseXml(xml, type, signType) {
    ...
    switch (type) {
      case 'middleware_nativePay':
        break;
      default:
        if (json.return_code !== 'SUCCESS') throw new Error(json.return_msg || 'XMLDataError' + xml);
    }
    ...
    switch (type) {
      ...
      case 'sendgroupredpack':
        if (json.qqappid !== this.appid) throw new Error('qqappid不匹配');
  ...
  async _request(params, type, cert = false) {
    ...
    let {
      status,
      data
    } = await urllib.request(this.urls[type], pkg);
    if (status !== 200) throw new Error('request fail');
    this.log('receive data =>\r\n%s\r\n', data);

    return ['downloadbill', 'downloadfundflow', 'sendredpack'].indexOf(type) < 0 ? this._parseXml(data, type, params.sign_type) : this._parseJson(data, type);
  }
  ...
  middlewareForExpress(type = 'pay') {
    return async (req, res, next) => {
      ...
      try {
        ...
        req.weixin = await this._parseXml(req.body, 'middleware_' + type);
      } catch (err) {
  ...
  middleware(type = 'pay') {
    return async (ctx, next) => {
      ...
      try {
        ...
        ctx.request.weixin = await this._parseXml(ctx.request.body, 'middleware_' + type);
      } catch (err) {
